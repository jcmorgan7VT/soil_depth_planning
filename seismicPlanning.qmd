...
plot(iris.vsurf)
colnames(all_df[iris.vsurf$varselect.interp])
...
rf_error <- tibble(iris.predictions, "actual" = all_df[, 1]) %>% 
  mutate(diff = actual - iris.predictions) %>% 
  rename(pk = actual) %>% 
...
# ALE plot
    p_load(iml)
    predictor <- Predictor$new(model = rf_model, data = data, y = "your_target_variable")

    # Assuming 'rf_model' is your randomForest model and 'X' is your feature matrix
    # 'pred.fun' is a function that takes the model and new data and returns predictions
    pred_fun <- function(model, newdata) {
      predict(model, newdata)
    }

    # Plot ALE for a single feature (e.g., the 1st feature)
    ALEPlot(X = X, X.model = rf_model, pred.fun = pred_fun, J = 1)

    # Plot ALE for an interaction between two features (e.g., 1st and 2nd)
    ALEPlot(X = X, X.model = rf_model, pred.fun = pred_fun, J = c(1, 2))
```
