...
```{r}
plot(rast(k))
dem10 <- rast("/Users/johnmorgan/Documents/VT Research/HB_data/hbef_10mdem/hbef_10mdem.tif") %>% 
  crop(ext(w3_outline)) %>%
  mask(ext(w3_outline))

k_crop <- rast(k)$var1.pred_var1.pred %>% 
  crop(ext(w3_outline)) %>%
  mask(ext(w3_outline))

k_resamp <- resample(k_crop, dem10, method = "bilinear")
plot(dem10 - k_resamp)

writeRaster(dem10 - k_resamp, "krig_output.tif", overwrite = TRUE, NAflag=-9999, gdal="COMPRESS=NONE")

filled_krig_out <- "filled_krig.tif"

wbt_fill_depressions(dem = "krig_output.tif",
                     output = filled_krig_out)
plot(rast(filled_krig_out) - dem10 + k_resamp, main = "10m surface depressions")
plot(k_resamp, main = "10m subsurface depressions")
```
